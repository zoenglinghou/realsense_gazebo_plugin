...
void RealSensePlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  do {
    ...
    if (name == "depthUpdateRate")
      ...
      cameraParamsMap_[DEPTH_CAMERA_NAME].camera_info_topic_name =
          _sdf->GetValue()->GetAsString();
    else if (name == "depthNoiseStddev")
      _sdf->GetValue()->Get(depthNoiseStddev_);
    else if (name == "depthDropout")
      _sdf->GetValue()->Get(depthDropout_);
    ...
    else if (name == "pointCloudInvalidValue")
      _sdf->GetValue()->Get(pointCloudInvalidValue_);
    ...
    else if (name == "pointCloudIsDense")
      _sdf->GetValue()->Get(pointCloudIsDense_);
    else if (name == "pointCloudColor")
      _sdf->GetValue()->Get(pointCloudColor_);
    else if (name == "prefix")
  ...
  this->nDist_ = std::normal_distribution<>(0, this->depthNoiseStddev_);
}
